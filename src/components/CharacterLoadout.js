...
import {CharacterKey, receiveCharacter, requestCharacter} from "../actions/armory/characters";
import {Item} from "./Item";
import Slot from "../concepts/Slot";
import {getCharacter} from "../reducers";


class CharacterLoadout extends Component {
  render() {
    // TODO:: how does this get in?
    let character = getCharacter(this.props, new CharacterKey("us", "azuremyst", "magicnumber"));
    if (character === undefined) {
      return <div>spinner here?</div>;
    }

    let backgroundUrl = character.thumbnail.replace('avatar', 'main');

    let backgroundStyle = {
      backgroundImage:`url("https://render-us.worldofwarcraft.com/character/${backgroundUrl}")`,
    ...
    return (
      ...
      <div style={backgroundStyle}>
        <div style={flexStyle}>
          <ul style={{flexDirection: "column", justifyContent: "space-around", flexGrow: 2}}>
            <Item slot={Slot.Head} item={character.items.head}/>
            <Item slot={Slot.Neck} item={character.items.neck}/>
            <Item slot={Slot.Shoulder} item={character.items.shoulder}/>
            <Item slot={Slot.Back} item={character.items.back}/>
            <Item slot={Slot.Chest} item={character.items.chest}/>
            <Item slot={Slot.Shirt} item={character.items.shirt}/>
            <Item slot={Slot.Tabard} item={character.items.tabard}/>
            <Item slot={Slot.Wrist} item={character.items.wrist}/>
          </ul>
          <div style={{flexGrow: 5, minWidth: "450px"}}>
          </div>
          <ul style={{flexDirection: "column", justifyContent: "space-around", flexGrow: 2}}>
            <Item slot={Slot.Hands} item={character.items.hands}/>
            <Item slot={Slot.Waist} item={character.items.waist}/>
            <Item slot={Slot.Legs} item={character.items.legs}/>
            <Item slot={Slot.Feet} item={character.items.feet}/>
            <Item slot={Slot.Finger} item={character.items.finger1}/>
            <Item slot={Slot.Finger} item={character.items.finger2}/>
            <Item slot={Slot.Trinket} item={character.items.trinket1}/>
            <Item slot={Slot.Trinket} item={character.items.trinket2}/>
          </ul>
        </div>
        <div>
          <ul style={{display: "flex", flexDirection: "row", justifyContent: "center"}}>
            <Item style={{flexDirection: "row-reverse"}} slot={Slot.MainHand} item={character.items.mainHand}/>
            <Item slot={Slot.OffHand} item={character.items.offHand}/>
          </ul>
        </div>
      </div>
    );
  ...
  componentDidMount() {
    this.props.requestCharacter(new CharacterKey("US", "azuremyst", "magicnumber"));
  }
}
...
const mapDispatchToProps = (dispatch) => {
  return {
    requestCharacter: (characterKey) => {
      dispatch(receiveCharacter(characterKey, magicnumber));
      // dispatch(requestCharacter(characterKey));
    }
  }
};

export default connect(mapStateToProps, mapDispatchToProps)(CharacterLoadout);